...
void Dir_GPIO_init(void)
{
 ...
 GPIO_InitStructure.GPIO_Pin = Dir_check2_pin|Dir_check3_pin;				 //LED0-->PB.5 端口配置
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 		 //推挽输出
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(Dir_check_port, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.5
	
 GPIO_InitStructure.GPIO_Pin = Dir_check1_pin;				 //LED0-->PB.5 端口配置
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 		 //推挽输出
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
...
u8 Dir_value(void)
{
	u8 t=0,t1=0,t2=0,t3=0;
	t  = GPIO_ReadInputDataBit(GPIOB,Dir_check1_pin);
