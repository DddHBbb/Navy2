...
int show_time;
float Temp_Left_blow=0,Temp_Left_suck=0,Temp_Right_blow=0,Temp_Right_suck=0,k=0;
int Right_suck_Bai=0,Right_suck_Shi=0,Right_suck_Ge=0;

float Temp_water=0,Temp_air=0;
...
if(TIM_GetITStatus(TIM6, TIM_IT_Update) != RESET)
	...
	if((Record.set_temp <= 34) && (Record.set_temp != 0))
		{
			k = 31.4;
		}
	else if((Record.set_temp > 34) && (Record.set_temp < 40))
		{
			k = 30.7;
		}
	else if(Record.set_temp >= 40)
		{
			k = 31.6;
		}	if(Samp_Flag == 1)
		{
